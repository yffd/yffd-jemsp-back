...
public class CustomPasswordEncoder implements PasswordEncoder {
    ...
    public String encode(CharSequence rawPassword) {
        try {
            ...
            LOG.info("encode方法：加密前（ {} ），加密后（ {} ）", rawPassword, pwd);
            return pwd;
        } catch (Exception e) {
            LOG.error("对密码进行md5异常", e);
            return null;
        }
    }
    ...
    public boolean matches(CharSequence rawPassword, String encodedPassword) {
        // 判断密码为空,直接返回false
        if (StringUtils.isBlank(rawPassword)) return false;
        //调用上面的encode 对请求密码进行MD5处理
        String pass = this.encode(rawPassword.toString());
        LOG.info("matches方法：请求密码为：{} ，数据库密码为：{}，加密后的请求密码为：{}", rawPassword, encodedPassword, pass);
        //比较密码是否相等
